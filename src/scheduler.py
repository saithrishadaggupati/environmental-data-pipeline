...
def run_pipeline():
    ...
    subprocess.run([sys.executable, "-m", "src.ingestion.fetch_aqi"])

    print("Step 2: Cleaning data...")
    subprocess.run([sys.executable, "-m", "src.transformation.clean_aqi"])

    print("Step 3: Loading into PostgreSQL...")
    subprocess.run([sys.executable, "-m", "src.loading.load_to_postgres"])
